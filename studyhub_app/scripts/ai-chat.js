...
class AIChatManager {
  ...
  updateSubjectInfo() {
    ...
    // Compter les QCM pour cette matière
    const subjectQCMs = Object.values(qcmData).filter(qcm => 
      qcm.subject === this.currentSubject.name
    );
    const totalQCMQuestions = subjectQCMs.reduce((sum, qcm) => sum + (qcm.questions?.length || 0), 0);
    
    // Compter les flashcards pour cette matière
    const subjectFlashcards = flashcards.filter(card => card.subject === this.currentSubject.name);
    
    // Compter les résumés pour cette matière
    const subjectResumes = Object.values(resumes).filter(resume => 
      resume.subject === this.currentSubject.name
    );
    
    // Compter les documents importés pour cette matière
    const subjectDocuments = importedFiles.filter(file => file.subject === this.currentSubject.name);

    document.getElementById('doc-count').textContent = subjectDocuments.length;
    document.getElementById('qcm-count').textContent = totalQCMQuestions;
  ...
  updateContextContent() {
    ...
    // Récupérer les QCM pour cette matière
    const subjectQCMs = Object.values(qcmData).filter(qcm => 
      qcm.subject === this.currentSubject.name
    );
    
    // Récupérer les flashcards pour cette matière
    const subjectFlashcards = flashcards.filter(card => card.subject === this.currentSubject.name);
    
    // Récupérer les résumés pour cette matière
    const subjectResumes = Object.values(resumes).filter(resume => 
      resume.subject === this.currentSubject.name
    );
    
    // Récupérer les documents importés pour cette matière
    const subjectDocuments = importedFiles.filter(file => file.subject === this.currentSubject.name);

    let contextHTML = `
      <div class="context-section">
        <h5>Documents importés (${subjectDocuments.length})</h5>
    ...
    contextHTML += `
      ...
      <div class="context-section">
        <h5>Résumés disponibles (${subjectResumes.length})</h5>
        <div class="context-items">
    `;
    ...
    subjectQCMs.forEach(qcm => {
      qcm.questions?.forEach(q => {
        const words = q.text.split(' ').filter(word => 
