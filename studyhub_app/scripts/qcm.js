...
class QCMManager {
  ...
  loadQCMsForPractice() {
    ...
    // Récupérer aussi les QCM générés automatiquement
    const generatedQCMs = Object.values(qcmData).filter(qcm => 
      qcm.subject === subject && qcm.generated
    );
    
    const allQCMs = [...qcms, ...generatedQCMs];
    
    const qcmList = document.getElementById('qcm-list');
    qcmList.innerHTML = '';
    
    if (allQCMs.length === 0) {
      qcmList.innerHTML = '<p class="text-center">Aucun QCM disponible pour cette matière. Importez un document pour générer des QCM automatiquement !</p>';
      return;
    }
    ...
    allQCMs.forEach(qcm => {
      ...
      qcmCard.innerHTML = `
        ...
        <div class="qcm-meta">
          ...
          <span><i class="fas fa-play"></i> ${qcm.attempts || 0} tentatives</span>
          <span><i class="fas fa-percentage"></i> ${qcm.avgScore || 0}%</span>
          ${qcm.generated ? '<span><i class="fas fa-robot"></i> Généré par IA</span>' : ''}
        </div>
