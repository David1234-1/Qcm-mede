...
class FlashcardManager {
  ...
  startStudySession(subject) {
    ...
    if (this.flashcards.length === 0) {
      if (window.StudyHub && window.StudyHub.NotificationManager) {
        window.StudyHub.NotificationManager.show('Aucune flashcard disponible pour cette matière. Importez un document pour générer des flashcards automatiquement !', 'warning');
      }
      return;
    }
